package by.it.group410972.loziuk.lesson12;
...
public class MySplayMap implements NavigableMap<Integer, String> {
    ...
    private void headMap(Node node, Integer toKey, boolean inclusive, MySplayMap result) {
        ...
        if ((inclusive && node.key.compareTo(toKey) <= 0) || (!inclusive && node.key.compareTo(toKey) < 0)) {
    ...
    private void tailMap(Node node, Integer fromKey, boolean inclusive, MySplayMap result) {
        ...
        if ((inclusive && node.key.compareTo(fromKey) >= 0) || (!inclusive && node.key.compareTo(fromKey) > 0)) {
            result.put(node.key, node.value);
        }
