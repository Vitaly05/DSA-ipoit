package by.it.group410972.loziuk.lesson15;
...
public class SourceScannerB {
    public static void main(String[] args) {
        ...
        try {
            Files.walk(srcPath).filter(p -> p.toString().endsWith(".java")).forEach(p -> {
                try {
                    String text;
                    ...
                    results.add(new FileResult(srcPath.relativize(p).toString(), bytes.length));
                } catch (IOException e) {
                    System.err.println("Ошибка при чтении файла: " + p + " (" + e.getMessage() + ")");
                }
            });

            results.sort(Comparator.comparingLong(FileResult::size).thenComparing(FileResult::path));
